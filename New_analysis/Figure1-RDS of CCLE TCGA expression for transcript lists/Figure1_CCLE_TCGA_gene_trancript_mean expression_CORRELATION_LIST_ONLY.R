expression_gene <- readRDS("Data/Figure1_CCLE_Gene_All_correlationlist.rds")
# Step 1: Read the list of allowed DepmapModelTypes
tcga_model_types <- read.table("Data/TCGA_DepmapModelType.txt", header = TRUE, stringsAsFactors = FALSE)
# Optional: View it
head(tcga_model_types)
# Step 2: Subset expression_gene to keep only matching DepmapModelType values
# Assuming the column is named 'DepmapModelType' in both files:
expression_gene <- expression_gene %>%
  filter(DepmapModelType %in% tcga_model_types$DepmapModelType)


transcript_expression <- readRDS("Data/Figure1_CCLE_transcript_ALL_correlation_list.rds")
...
df_expr <- expression_gene[, c(3, 4:567)]

# Step 2: Calculate median expression per sample (row-wise) across gene columns only
df_expr <- df_expr %>%
  rowwise() %>%
  mutate(median_expr = median(c_across(-DepmapModelType), na.rm = TRUE)) %>%
  ungroup()

# Step 3: Create a plotting data frame
plot_df <- df_expr %>%
  select(DepmapModelType, median_expr) %>%
...
df_expr <- transcript_expression[, c(3, 4:1181)]
...
long_expr <- expression_gene %>%
  pivot_longer(cols = 4:567, names_to = "TranscriptID", values_to = "Expression")
...
long_expr <- transcript_expression %>%
  pivot_longer(cols = 4:1181, names_to = "TranscriptID", values_to = "Expression")
...
ggplot(skcm_ranks_long, aes(x = Rank, fill = RankType)) +
  ...
  coord_cartesian(xlim = c(1, 16), ylim = c(0, 1.0)) +  # Add ylim here
  theme_minimal() +
  theme(legend.position = "none")

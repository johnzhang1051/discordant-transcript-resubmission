...
# If you're using data.table, make sure it's loaded
library(data.table)
# Ensure expr_df is a data.table
setDT(expr_df)
# Modify the 'samples' column to remove the period and everything after it
expr_df[, sample := sub("\\..*", "", sample)]


TCGA_phenotype <- read.csv("Data/TCGA_phenotype.csv")
final_genes <- read.csv("Data/final_genes.csv")

# === 2. FILTER & JOIN ===
colnames(expr_df)[1] <- "gene_id"
expr_annotated <- inner_join(final_genes, expr_df, by = "gene_id")


# === 3. ADD PRIMARY DISEASE AS FIRST ROW ===
sample_cols <- setdiff(colnames(expr_annotated), c("gene_id", "Gene"))
...
expr_data <- expr_data %>% mutate(across(-c(gene_id, Gene), as.numeric))
long_expr <- expr_data %>%
  pivot_longer(cols = -c(gene_id, Gene), names_to = "sample", values_to = "expression") %>%
...
pdf("gene_expression_by_disease.pdf", width = 10, height = 6)
gene_ids <- unique(long_expr$gene_id)

for (tid in gene_ids) {
  df <- long_expr %>% filter(gene_id == tid)
  if (nrow(df) == 0) next
...
ranked_df <- long_expr %>%
  group_by(gene_id, primary_disease) %>%
...
### code for Figure 1C ABR gene expression TCGA
expr_annotated_with_disease <- rbind(new_row_df, expr_annotated)
...
### ABR expression
saveRDS(expr_annotated_with_disease, "TCGA_gene_disease_Figure1B.rds")
